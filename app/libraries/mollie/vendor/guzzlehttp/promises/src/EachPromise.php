 ...
 */
class EachPromise implements \_PhpScoper5c0568000558c\GuzzleHttp\Promise\PromisorInterface
{
    private $pending = [];
    /** @var \Iterator */
    private $iterable;
    ...
    {
        $this->mutex = \false;
        $this->aggregate = new \_PhpScoper5c0568000558c\GuzzleHttp\Promise\Promise(function () {
            \reset($this->pending);
            ...
            while ($promise = \current($this->pending)) {
                \next($this->pending);
                $promise->wait();
                if ($this->aggregate->getState() !== \_PhpScoper5c0568000558c\GuzzleHttp\Promise\PromiseInterface::PENDING) {
                    return;
                }
            }
        });
