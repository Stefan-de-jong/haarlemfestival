...
<style> footer{margin-top: -15px;}</style>
    ...
    <section class="text-left" id="main-content" style="position: absolute;background-color: rgb(255,104,104);width: 1156px;height: 979px;margin-left: 384px;">
        ...
        <?php $y = 0; $x = 0; $count = 0; $rows_2_photos = 0; //y is used for top-margin, x for left-margin ?>
        <?php foreach ($data['artists'] as $artist) :?>
        <?php $rows_2_photos = intval($count / 2); $y = 250.86 * $rows_2_photos; //look how many rows of 2 pictures there are, increase x with the amount of rows?>
        <img id="pic<?php echo $artist->getId();?>" src="<?php echo URLROOT; ?>/img/dance/artist_image_<?php echo $artist->getId();?>.png" style="position: absolute; width: 578px; margin-top: <?php echo $y ?>px; margin-left: <?php echo $x ?>px;" onclick = "loadPanel(<?php echo $artist->getId();?>)"> <?php $count++; ?>
        <?php if ($x == 0) {$x = 578;} else {$x = 0;} //if x = 0 make it 578px else make it 0px (it is necessary to do this, since the picture after this picture is either on the left of the previous picture or on the right) ?>
         <?php endforeach; ?>
