...
class CMSRepo
{
    ...
    public function __construct()
    {
        ...
        '6155ea87c23c52518df731aaa1f635aa' =>
            ['table' => 'restaurant', 'updateAbles' => ['name', 'stars', 'address'], 'readOnly' => false, 'idColumn' => 'id'],
        '7cda127b9c7c0fa6430b710f04d0b08f' => ['table' =>
            'event', 'updateAbles' => ['date', 'begin_time', 'end_time'], 'readOnly' => false, 'idColumn' => 'id', 'customQuery' => $this->customQueryDance()],
    ...
    private function resetPasswordCustomer($id)
    {
        $this
            ->db
            ->query('select email from customer where id = :id');
        $this
            ->db
            ->bind(':id', $id);
        $em = $this
        ...
        if ($emailTakenCustomer)
        {
            ...
            $this
                ->db
                ->query('UPDATE customer SET password = :pass WHERE email = :em');
            $this
                ->db
                ->bind(':pass', password_hash($newpass, PASSWORD_DEFAULT));
            $this
                ->db
                ->bind(':em', $em);
            $this
    ...
    private function customQueryDance()
    {
        return "SELECT event.id,artist_name, v.venue_name, date, begin_time,end_time FROM event
    ...
    private function getHistoricSnippets2()
    {
        $this
            ->db
            ->query('SELECT *  FROM tourlocation');
        return $this
            ->db
            ->resultSet();
    }
    ...
    private function getVolunteerSnippets()
    {
        $this
            ->db
            ->query('SELECT *  FROM volunteersnippet');
        return $this
            ->db
            ->resultSet();
    }
    public function getAllSnippets()
    {
        try
        {
            ...
            $volunteerSnippetsRaw = $this->getVolunteerSnippets();
            $danceSnippets = [];
            ...
            $volunteerSnippets = [];
            foreach ($volunteerSnippetsRaw as $k => $v)
            {
                $f = new FormattedSnippet();
                $f->cat = 6;
                $f->title = "";
                $f->val = $v->text;
                $f->id = $v->id;
                array_push($volunteerSnippets, $f);
            }
            return ["dance" => $danceSnippets, "historicMain" => $historicSnippets1, "historicRoute" => $historicSnippets2,
                "foodMain" => $foodSnippets1, "foodRestaurants" => $foodSnippets2, 'volunteer'=>$volunteerSnippets];
        }
        catch(Exception $e)
        {
            return [];
